...
* Consider the use of _prepared statements_ in your code client when using the same queries over and over again, both for performance and security (escaping).
...
* Mind the rights. PostgreSQL for example does not allow granting `ALTER` statements. This might affect your ideas of having a limited user while also trying to do schema migrations under the same user. Changing an owner afterwards will not be easy as well, as you would have to touch all your backups as well.
