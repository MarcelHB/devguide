...
* [Sutton's law](https://en.wikipedia.org/wiki/Sutton%27s_law), also close to Occam &ndash; More on debugging at another time. Yet it helps well in a certain kind of situation: There is a bug or failure, and you are strongly biased before doing the obvious tests first as there is some assumption regarding the cause in your mind already. In the case when it is not confirmed, think of this law instead of digging around close to your first point of entry, chances are good to have something more obvious then instead.
