...
* There is also [UTF-16](https://de.wikipedia.org/wiki/UTF-16), requiring two to four bytes. It requires less magic to bit patterns but also requires more space. _Be careful_, there may be a _byte-order marker_ (BOM) of 2 bytes in use at the beginning of the text. It tells you which one of the two bytes is supposed to be the _lower_ one. Microsoft made Windows use UTF-16 for a very long time, requiring developers to constantly convert on touch points with UTF-8 data.
* For [UTF-32](https://de.wikipedia.org/wiki/UTF-32), there are no reserved bits, since every element is using exactly four bytes, thus wasting a lot of space for `0x00`s in Latin languages, but simplifying all related string operations. UTF-32 may also come with a BOM.
* Make use of UTF-8 unless there is a very specific requirement.
* Programming languages may use other approaches to encode text internally, see [Python](https://peps.python.org/pep-0393/) for such an example.
...
* Falling for _microservices_ for the wrong reasons (some article on _medium.com_?); and just maybe use the term _micro_ less, since most actually are not so micro anyway. Good and very good reasons to have a diverse range of _services_:
  ...
  Signs that some guys maybe went for that hype too early:
    ...
    * Not knowing how to trace behavior across services safely, fuzzy ways to correlate errors (have a look at [OpenTelemetry](https://opentelemetry.io/docs/)).
